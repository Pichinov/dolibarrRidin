...
require_once DOL_DOCUMENT_ROOT . '/stripe2/config.php'; // This set stripe2 global env
 ...
 */
class Stripe2 extends CommonObject
{
	/**
	 * @var int ID
	 */
	...
	/**
	 * @var string
	 * @deprecated Was used by createPaymentStripe2 only that is deprecated
	 */
	public $result;
	...
	/**
	 * Return main company OAuth Connect Stripe2 account
	 *
	 * @param 	string	$mode		'Stripe2Test' or 'Stripe2Live'
	 * @param	int		$fk_soc		Id of thirdparty
	 * @param	int		$entity		Id of entity (-1 = current environment)
	 * @return 	string				Stripe2 account 'acc_....' or '' if no OAuth token found
	 */
	public function getStripe2Account($mode = 'StripeTest', $fk_soc = 0, $entity = -1)
	{
		global $conf;
		...
		dol_syslog(get_class($this) . "::getStripe2Account", LOG_DEBUG);
		...
		dol_syslog("No dedicated stripe2 Connect account available for entity " . $conf->entity);
	 ...
	 */
	public function getStripe2CustomerAccount($id, $status = 0, $site_account = '')
	{
		include_once DOL_DOCUMENT_ROOT . '/societe/class/societeaccount.class.php';
		$societeaccount = new SocieteAccount($this->db);
		return $societeaccount->getCustomerAccount($id, 'stripe2', $status, $site_account); // Get thirdparty cus_...
	}


	/**
	 * Get the stripe2 customer of a thirdparty (with option to create it in stripe2 if not linked yet).
	 * Search on site_account = 0 or = $stripe2arrayofkeysbyenv[$status]['publishable_key']
	 *
	 * @param	Societe|Adherent	$object				Object thirdparty to check, or create on stripe2 (create on stripe2 also update the stripe_account table for current entity).  Used for AdherentType and Societe.
	 * @param	string		$key							''=Use common API. If not '', it is the stripe2 connect account 'acc_....' to use stripe2 connect
	 ...
	 */
	public function customerStripe2($object, $key = '', $status = 0, $createifnotlinkedtostripe2 = 0)
	{
		global $conf, $user;

		if (empty($object->id)) {
			dol_syslog("customerStripe2 is called with the parameter object that is not loaded");
			return null;
		}
		...
		$sql .= " AND (sa.site_account IS NULL OR sa.site_account = '' OR sa.site_account = '" . $this->db->escape($stripe2arrayofkeysbyenv[$status]['publishable_key']) . "')";
		...
		dol_syslog(get_class($this) . "::customerStripe2 search stripe2 customer id for thirdparty id=" . $object->id, LOG_DEBUG);
		...
		if ($resql) {
			...
			if ($num) {
				...
				dol_syslog(get_class($this) . "::customerStripe2 found stripe2 customer key_account = " . $tiers . ". We will try to read it on Stripe2 with publishable_key = " . $stripe2arrayofkeysbyenv[$status]['publishable_key']);
			...
			} elseif ($createifnotlinkedtostripe2) {
				...
				// We force data to "null" if not defined as expected by Stripe2
				if (empty($vatcleaned)) $taxinfo=null;
				$dataforcustomer["tax_info"] = $taxinfo;
				*/

				//$a = \Stripe\Stripe2::getApiKey();
				...
				try {
					...
					// Create the VAT record in Stripe2
					if (getDolGlobalString('STRIPE2_SAVE_TAX_IDS')) {	// We setup to save Tax info on Stripe2 side. Warning: This may result in error when saving customer
						if (!empty($vatcleaned)) {
					...
					$sql .= " VALUES (" . ((int) $object->id) . ", '', '" . $this->db->escape($customer->id) . "', 'stripe', '" . $this->db->escape($stripe2arrayofkeysbyenv[$status]['publishable_key']) . "', " . ((int) $status) . ", " . ((int) $conf->entity) . ", '" . $this->db->idate(dol_now()) . "', " . ((int) $user->id) . ")";
					$resql = $this->db->query($sql);
					if (!$resql) {
	...
	/**
	 * Get the stripe2 payment method Object from its ID
	 *
	 ...
	 */
	public function getPaymentMethodStripe2($paymentmethod, $key = '', $status = 0)
	{
		$stripe2paymentmethod = null;
		...
		try {
			...
			if (empty($key)) {				// If the Stripe2 connect account not set, we use common API usage
				...
				$stripe2paymentmethod = \Stripe\PaymentMethod::retrieve((string) $paymentmethod->id, array("stripe_account" => $key));
			}
		} catch (Exception $e) {
			$this->error = $e->getMessage();
		}

		return $stripe2paymentmethod;
	}

	/**
	 * Get the stripe2 reader Object from its ID
	 *
	 * @param	string	$reader	   			Reader ID
	 * @param	string	$key				''=Use common API. If not '', it is the stripe2 connect account 'acc_....' to use stripe2 connect
	 * @param	int	$status				Status (0=test, 1=live)
	 * @return 	\Stripe\Terminal\Reader|null		stripe2 Reader or null if not found
	 */
	public function getSelectedReader($reader, $key = '', $status = 0)
	{
		...
		try {
			...
			if (empty($key)) {				// If the stripe2 connect account not set, we use common API usage
				...
				$stripe2paymentmethod = \Stripe\Terminal\Reader::retrieve((string) $reader, array("stripe_account" => $key));
			}
		} catch (Exception $e) {
			$this->error = $e->getMessage();
		}
	...
	public function getPaymentIntent($amount, $currency_code, $tag, $description = '', $object = null, $customer = null, $key = null, $status = 0, $usethirdpartyemailforreceiptemail = 0, $mode = 'automatic', $confirmnow = false, $payment_method = null, $off_session = 0, $noidempotency_key = 1, $did = 0)
	{
		...
		if (!in_array($currency_code, $arrayzerounitcurrency)) {
			$stripe2amount = $amount * 100;
		} else {
			$stripe2amount = $amount;
		}

		$fee = 0;
		if (getDolGlobalString("STRIPE2_APPLICATION_FEE_PERCENT")) {
			$fee = $amount * ((float) getDolGlobalString("STRIPE2_APPLICATION_FEE_PERCENT", '0') / 100) + (float) getDolGlobalString("STRIPE2_APPLICATION_FEE", '0');
		}
		if ($fee >= (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MAXIMAL", '0') && (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MAXIMAL", '0') > (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MINIMAL", '0')) {
			$fee = (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MAXIMAL", '0');
		} elseif ($fee < (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MINIMAL", '0')) {
			$fee = (float) getDolGlobalString("STRIPE2_APPLICATION_FEE_MINIMAL", '0');
		}
		if (!in_array($currency_code, $arrayzerounitcurrency)) {
			$stripe2fee = round($fee * 100);
		} else {
			$stripe2fee = round($fee);
		}

		$paymentintent = null;

		if (is_object($object) && getDolGlobalInt('STRIPE2_REUSE_EXISTING_INTENT_IF_FOUND') && !getDolGlobalInt('STRIPE2_CARD_PRESENT')) {
			// Warning. If a payment was tried and failed, a payment intent was created.
			// But if we change something on object to pay (amount or other that does not change the idempotency key), reusing same payment intent, is not allowed by Stripe2.
			// Recommended solution is to recreate a new payment intent each time we need one (old one will be automatically closed by Stripe2 after a delay), Stripe2 will
			...
			dol_syslog(get_class($this) . "::getPaymentIntent search stripe2 payment intent for object id = " . $object->id, LOG_DEBUG);
			$resql = $this->db->query($sql);
			if ($resql) {
				$num = $this->db->num_rows($resql);
				if ($num) {
		...
		if (empty($paymentintent)) {
			...
			if (getDolGlobalInt('STRIPE2_SEPA_DIRECT_DEBIT')) {
				$paymentmethodtypes[] = "sepa_debit"; //&& ($object->thirdparty->isInEEC())
				//$descriptor = preg_replace('/ref=[^:=]+/', '', $descriptor);	// Clean ref
			}
			if (getDolGlobalInt('STRIPE2_KLARNA')) {
				$paymentmethodtypes[] = "klarna";
			}
			if (getDolGlobalInt('STRIPE2_BANCONTACT')) {
				$paymentmethodtypes[] = "bancontact";
			}
			if (getDolGlobalInt('STRIPE2_IDEAL')) {
				$paymentmethodtypes[] = "ideal";
			}
			if (getDolGlobalInt('STRIPE2_GIROPAY')) {
				$paymentmethodtypes[] = "giropay";
			}
			if (getDolGlobalInt('STRIPE2_SOFORT')) {
				$paymentmethodtypes[] = "sofort";
			}
			if (getDolGlobalInt('STRIPE2_CARD_PRESENT') && $mode == 'terminal') {
			...
			if ($descriptor) {
				$dataforintent["statement_descriptor_suffix"] = $descriptor; // For card payment, 22 chars that appears on bank receipt (prefix into stripe2 setup + this suffix)
			...
			if ($conf->entity != getDolGlobalInt('STRIPE2CONNECT_PRINCIPAL') && $stripe2fee > 0) {
				$dataforintent["application_fee_amount"] = $stripe2fee;
			}
			...
			try {
				...
				// Note: If all data for payment intent are same than a previous on, even if we use 'create', stripe2 will return ID of the old existing payment intent.
				if (!empty($key)) {				// If the stripe2 connect account not set, we use common API usage
					$arrayofoptions["stripe_account"] = $key;
				}

				dol_syslog(get_class($this) . "::getPaymentIntent " . $stripe2arrayofkeysbyenv[$status]['publishable_key'], LOG_DEBUG);
				...
				if (is_object($object)) {
					...
					$_SESSION["stripe2_payment_intent"] = $paymentintent;
				}
			} catch (Stripe\Exception\CardException $e) {
				$error++;
				$this->error = $e->getMessage();
				$this->code = $e->getStripe2Code();
	...
	/**
	 * Get the Stripe2 payment intent. Create it with confirmnow=false
	 * Warning. If a payment was tried and failed, a payment intent was created.
	...
	public function getSetupIntent($description, $object, $customer, $key, $status, $usethirdpartyemailforreceiptemail = 0, $confirmnow = false)
	{
		...
		if (empty($setupintent)) {  // @phan-suppress-current-line PhanPluginConstantVariableNull
			...
			if (getDolGlobalString('STRIPE2_BANCONTACT')) {
				$paymentmethodtypes[] = "bancontact";
			}
			if (getDolGlobalString('STRIPE2_IDEAL')) {
				$paymentmethodtypes[] = "ideal";
			}
			// Giropay not possible for setup intent
			if (getDolGlobalString('STRIPE2_SOFORT')) {
				$paymentmethodtypes[] = "sofort";
			}
			...
			try {
				...
				// Note: If all data for payment intent are same than a previous one, even if we use 'create', stripe2 will return ID of the old existing payment intent.
				if (empty($key)) {				// If the stripe2 connect account not set, we use common API usage
					//$setupintent = \stripe2\SetupIntent::create($dataforintent, array("idempotency_key" => "$description"));
				...
				{
					$_SESSION["stripe2_setup_intent"] = $setupintent;
				}*/
			} catch (Exception $e) {
				//var_dump($dataforintent);
				//var_dump($description);
	 ...
	 */
	public function cardStripe2($cu, CompanyPaymentMode $object, $stripe2acc = '', $status = 0, $createifnotlinkedtostripe2 = 0)
	{
		...
		dol_syslog(get_class($this) . "::cardStripe2 search stripe2 card id for paymentmode id=" . $object->id . ", stripe2acc=" . $stripe2acc . ", status=" . $status . ", createifnotlinkedtostripe2=" . $createifnotlinkedtostripe2, LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			$num = $this->db->num_rows($resql);
			if ($num) {
				...
				dol_syslog(get_class($this) . "::cardStripe2 cardref=" . $cardref);
				if ($cardref) {
					try {
					...
					try {
						if (empty($stripe2acc)) {				// If the stripe2 connect account not set, we use common API usage
							if (!getDolGlobalString('STRIPE2_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
								dol_syslog("Try to create card with dataforcard = " . json_encode($dataforcard));
								$card = $cu->sources->create($dataforcard);
								if (!$card) {
									$this->error = 'Creation of card on Stripe2 has failed';
								}
							} else {
								$connect = '';
								if (!empty($stripe2acc)) {
								...
								$urtoswitchonstripe2 = '<a href="' . $url . '" target="_stripe2">' . img_picto($langs->trans('ShowInStripe2'), 'globe') . '</a>';

								//dol_syslog("Error: This case is not supported", LOG_ERR);
								$this->error = str_replace('{s1}', $urtoswitchonstripe2, $langs->trans('CreationOfPaymentModeMustBeDoneFromStripe2Interface', '{s1}'));
							}
						} else {
							if (!getDolGlobalString('STRIPE2_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
								dol_syslog("Try to create card with dataforcard = " . json_encode($dataforcard));
								$card = $cu->sources->create($dataforcard, array("stripe_account" => $stripe2acc));
								if (!$card) {
									$this->error = 'Creation of card on Stripe2 has failed';
								}
							} else {
	 ...
	 */
	public function sepaStripe2($cu, CompanyPaymentMode $object, $stripe2acc = '', $status = 0, $createifnotlinkedtostripe2 = 0)
	{
		global $conf;
		$sepa = null;
		...
		dol_syslog(get_class($this) . "::sepaStripe2 search stripe2 ban id for paymentmode id=" . $object->id . ", stripe2acc=" . $stripe2acc . ", status=" . $status . ", createifnotlinkedtostripe2=" . $createifnotlinkedtostripe2, LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			$num = $this->db->num_rows($resql);
			if ($num) {
				...
				dol_syslog(get_class($this) . "::sepaStripe2 paymentmode=" . $cardref);
				...
				if ($cardref) {
					...
					try {
						...
						// Force to use the correct API key
						global $stripe2arrayofkeysbyenv;
						$stripe2acc = $stripe2arrayofkeysbyenv[$servicestatus]['secret_key'];

						dol_syslog("Try to create sepa_debit with data = " . json_encode($dataforcard));

						$s = new \Stripe\StripeClient($stripe2acc);

						//var_dump($dataforcard);exit;

						$sepa = $s->paymentMethods->create($dataforcard);
						if (!$sepa) {
							$this->error = 'Creation of payment method sepa_debit on Stripe2 has failed';
							...
							if (!$cs) {
								...
								$sql .= " stripe_account= '" . $this->db->escape($cu->id . "@" . $stripe2acc) . "',";
								$sql .= " ext_payment_site = '" . $this->db->escape($service) . "'";
								if (!empty($cs->mandate)) {
									$mandateservice = new \Stripe\Mandate($stripe2acc);
						...
						$sepa = null;
						$this->error = 'Stripe2 error: ' . $e->getMessage() . '. Check the BAN information.';
	 ...
	 * @deprecated
	 */
	public function createPaymentStripe2($amount, $currency, $origin, $item, $source, $customer, $account, $status = 0, $usethirdpartyemailforreceiptemail = 0, $capture = true)
	{
		global $conf;
		...
		try {
			...
			if (empty($conf->stripe2connect->enabled)) {	// With a common stripe2 account
				if (preg_match('/pm_/i', $source)) {
					$stripe2card = $source;
					$amountstripe2 = $stripe2amount;
					$FULLTAG = 'PFBO'; // Payment From Back Office
					$stripe2 = $return;
					$amounttopay = $amount;
					$servicestatus = $status;

					dol_syslog("* createPaymentstripe2 get stripe2acc", LOG_DEBUG);
					$stripe2acc = $stripe2->getStripe2Account($service); // Get Stripe2 OAuth connect account if it exists (no network access here)

					dol_syslog("* createPaymentStripe2 Create payment for customer " . $customer->id . " on source card " . $stripe2card->id . ", amounttopay=" . $amounttopay . ", amountstripe2=" . $amountstripe2 . ", FULLTAG=" . $FULLTAG, LOG_DEBUG);

					// Create payment intent and charge payment (confirmnow = true)
					$paymentintent = $stripe2->getPaymentIntent($amounttopay, $currency, $FULLTAG, $description, $invoice, $customer->id, $stripe2acc, $servicestatus, 0, 'automatic', true, $stripe2card->id, 1);
					...
					if ($paymentintent->status == 'succeeded') {
						...
						$charge->failure_code = $stripe2->code;
						$charge->failure_message = $stripe2->error;
						$charge->failure_declinecode = $stripe2->declinecode;
						$stripe2failurecode = $stripe2->code;
						$stripe2failuremessage = $stripe2->error;
						$stripe2failuredeclinecode = $stripe2->declinecode;
					}
				...
				// With Stripe Connect
				$fee = $amount * ($conf->global->STRIPE2_APPLICATION_FEE_PERCENT / 100) + $conf->global->STRIPE2_APPLICATION_FEE;
				if ($fee >= $conf->global->STRIPE2_APPLICATION_FEE_MAXIMAL && $conf->global->STRIPE2_APPLICATION_FEE_MAXIMAL > $conf->global->STRIPE2_APPLICATION_FEE_MINIMAL) {
					$fee = getDolGlobalString('STRIPE2_APPLICATION_FEE_MAXIMAL');
				} elseif ($fee < $conf->global->STRIPE2_APPLICATION_FEE_MINIMAL) {
					$fee = getDolGlobalString('STRIPE2_APPLICATION_FEE_MINIMAL');
				}
				...
				$paymentarray = array(
					...
					"currency" => "$currency",
					"statement_descriptor_suffix" => dol_trunc($description, 10, 'right', 'UTF-8', 1), // 22 chars that appears on bank receipt (company + description)
					"description" => "Stripe2 payment: " . $description,
					"capture"  => $capture,
					"metadata" => $metadata,
					"source" => "$source",
					"customer" => "$customer"
				);
				if ($conf->entity != $conf->global->STRIPE2CONNECT_PRINCIPAL && $stripe2fee > 0) {
					$paymentarray["application_fee_amount"] = $stripe2fee;
				}
				if ($societe->email && $usethirdpartyemailforreceiptemail) {
					$paymentarray["receipt_email"] = $societe->email;
				}
				...
				if (preg_match('/pm_/i', $source)) {
					...
					dol_syslog("* createPaymentStripe2 Create payment on card " . $stripe2card->id . ", amounttopay=" . $amounttopay . ", amountstripe2=" . $amountstripe2 . ", FULLTAG=" . $FULLTAG, LOG_DEBUG);
			...
			*/
			...
			if (preg_match('/pm_/i', $source)) {
				...
				if ($charge->source->type == 'card') {
					...
					$stripe2 = new Stripe2($this->db);
					$src = \Stripe\Source::retrieve((string) $charge->source->three_d_secure->card, array(
						"stripe_account" => $stripe2->getStripe2Account($service)
					));
