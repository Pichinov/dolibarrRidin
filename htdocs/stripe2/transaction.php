...
llxHeader('', $langs->trans("Stripe2TransactionList"));
...
if (!$rowid) {
	...
	$title = $langs->trans("Stripe2TransactionList");
	$title .= (!empty($stripe2acc) ? ' (Stripe2 connection with Stripe2 OAuth Connect account ' . $stripe2acc . ')' : ' (Stripe2 connection with keys from Stripe2 module setup)');
	...
	try {
		if ($stripe2acc) {
			$txn_all = \Stripe\BalanceTransaction::all(array("limit" => $limit), array("stripe_account" => $stripe2acc));
		} else {
		...
		foreach ($txn_all->data as $txn) {
			...
			if ($txn->type == 'stripe_fee' || $txn->type == 'reserve_transaction') {
				...
				print "<td><a href='" . $url . "' target='_stripe2'>" . img_picto($langs->trans('ShowInStripe2'), 'globe') . " " . $txn->source . "</a></td>\n";
			}
			...
			//MODIF JOSE
			if ($txn->fee > 0 && getDolGlobalInt('STRIPE2_AUTO_ADD_FEES_TO_BANK')) {


				stripe2AddPaymentFeeOnBank(
					$txn->source,
					$txn->fee / 100,
					$fk_account_stripe2,
					$txn->id,
