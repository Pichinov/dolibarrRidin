...
llxHeader('', $langs->trans("Stripe2PayoutList"));
...
/*if (empty($stripe2account))
{
	print $langs->trans('Errorstripe2AccountNotDefined');
}*/
...
if (!$rowid) {
	...
	$title = $langs->trans("Stripe2PayoutList");
	...
	try {
		if ($stripe2acc) {
			$payout_all = \Stripe\Payout::all(array("limit" => $limit), array("stripe_account" => $stripe2acc));
		} else {
		...
		foreach ($payout_all->data as $payout) {
			...
			print "<td><a href='" . $url . "' target='_stripe2'>" . img_picto($langs->trans('ShowInStripe2'), 'globe') . " " . $payout->id . "</a></td>\n";
